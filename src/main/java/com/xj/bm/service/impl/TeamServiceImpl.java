...
import com.xj.bm.contant.TeamStatusEnum;
...
import com.xj.bm.model.vo.UserVO;
import com.xj.bm.service.TeamService;
import com.xj.bm.mapper.TeamMapper;
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team>
    ...
    public List<TeamUserVO> listTeams(TeamQuery teamQuery, boolean isAdmin) {
        ...
        queryWrapper.and(qw ->qw
                .isNull("expireTime")
                .or()
                .gt("expireTime", LocalDateTime.now()));
        ...
        // 关联查询创建人的用户信息
        for (Team everyteam : teamList) {
            Long userId = everyteam.getUserId();
            if (userId == null) {
                continue;
            }
            User user = userService.getById(userId);
            TeamUserVO teamUserVO = new TeamUserVO();
            BeanUtils.copyProperties(everyteam, teamUserVO);
            // 脱敏用户信息
            if (user != null) {
    ...
    public boolean joinTeam(TeamJoinRequest teamJoinRequest, User loginUser) {
        ...
        // 只有一个线程能获取到锁
        RLock lock = redissonClient.getLock("bm:join_team"+teamId);
        //除了 teamId，还能怎么优化锁的粒度？
        /**
         * 当前基于teamId的粒度：
         * ✅ 能保证同一时刻只有一个线程能操作某个队伍 ，
         * ❌ 极少数情况下不控制用户并发的层面（同一用户加两个不同队伍导致最终加入了6个队伍）
         * <p>
         * 基于userId枷锁
         * ✅ 保证同一用户同一时间只能发起一次加队伍请求
         * ❌ 不能限制队伍“并发加入”导致人数超限
         * <p>
         * 基于 userId + teamId 组合加锁
         * ✅ 精准控制一个用户加一个队伍的并发
         * ✅ 用户可以并发加不同队伍（只要队伍 ID 不同）
         * ✅ 同一个队伍允许不同用户同时加，只要不会超
         * ❌ 仍需对“队伍是否满员”在代码逻辑上二次校验（防止瞬间并发穿透）
         * <p>
         * userId + teamId 双重锁
         * ✅ 用户只能一次性加入一个队
         * ✅ 队伍不会超员
